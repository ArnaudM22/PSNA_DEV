...
pars.edge_list(tonkean_focals.data, social_behaviors)
pars.adj_table(tonkean_focals.data, directed_interaction,
               social_behaviors, tonkean_focals.indiv, undir_adj_table=True)
# mettre en dict, dissocier les calculs de taux, l'empty diagonal.
behavior_rate_dict = pars.table_dict(tonkean_focals.data, affiliative_interactions,
                                     directed_interaction, social_behaviors, tonkean_focals.indiv, undir_adj_table=True)
...
dsi_tonk = constr.dsi_table(
    behavior_rate_list, tonkean_focals.indiv, affiliative_interactions, behav_obs, tot_obs)

pars.length_adj(tonkean_focals.data, tonkean_focals.indiv,
                tonkean_focals.affiliative_behaviors)
...
rhesus2022 = etho.Focals('../Data/Raw/Rhesus_2022_xls',
                         open_preprocessed=False, check_empty_col=True, ignore=())
...
for i in grooming.index:
    if (grooming.loc[i, 'Behavior'] == '2 Zone de Grooming'):
        print(re.search("Focal est (.*?)\|",
              grooming.loc[i, 'Modifiers']).group(1))
    print(i)
...
grooming.loc[grooming['Behavior'] == '2 Zone de Grooming',
             'Modifiers'].str.split("|", expand=True)[0]
