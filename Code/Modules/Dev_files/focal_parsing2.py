...
def adj_matrices(data, behaviors):
    ...
    inter_list = inter_list.reset_index()
    inter_list = inter_list.merge(inter_list.loc[:,'Other individual'].str.split(',', expand=True), left_index=True, right_index=True)  # The names in the modifiers section are separated.)
    inter_list = inter_list.replace('NaN', np.nan)
    inter_list2 = inter_list.loc[:, ['Behavior', 'Subject', 'Interaction direction', 'Other individual','Duration (s)', 0]].dropna().rename(
        columns={0: 'Other individual2'})  # undir_adj2 is used for the loop.
