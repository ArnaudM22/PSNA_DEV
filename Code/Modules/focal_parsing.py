...
def behaviors_list_def(data):
    affiliative_interactions = list(
        dict.fromkeys(data.loc[data['Social behavior category'] == 'Affiliative interaction', 'Behavior']))
    proximity_association = list(
        dict.fromkeys(data.loc[data['Social behavior category'] == 'Proximity association', 'Behavior']))
    agonistic_interactions = list(
        dict.fromkeys(data.loc[data['Social behavior category'] == 'Agonistic interaction', 'Behavior']))
    directed_interaction = list(
        dict.fromkeys(data.loc[data['Interaction direction'].isin(['Focal est emetteur', 'Focal est recepteur']), 'Behavior']))
    return affiliative_interactions, proximity_association, agonistic_interactions, directed_interaction


def ind_obs_time(data):
    ind_time_list = data.groupby('numfocal').first().groupby(
...
def edge_list(data, behaviors):
    ...
    inter_list.loc[:, ['Subject', 'Interaction direction', 'Other individual']] = inter_list.reset_index(
    ).loc[:, ['Subject', 'Interaction direction', 'Other individual']].values
    # Lines with "None" values are deleted.
    inter_list = inter_list.replace('None', np.nan).dropna(axis=0)
    inter_list.loc[inter_list['Interaction direction'] == 'Focal est recepteur', ['Other individual', 'Subject']] = inter_list.loc[inter_list['Interaction direction']
                                                                                                                                   == 'Focal est recepteur', ['Subject', 'Other individual']].values  # Subject and Modifiers are reversed to always have the sender as subject.
    ...
    inter_list = inter_list.groupby(
        ['Behavior', 'Subject', 'Other individual']).sum(numeric_only=True)
    return inter_list
...
def adj_table(data, directed_interaction, behaviors, indiv_list, undir_adj_table=False):
    ...
    adj = pd.DataFrame(index=index).merge(inter_list, how='left', on=[
        'Behavior', 'Subject', 'Other individual']).fillna(0).unstack(1)
...
def table_dict(data, rate_behavior, directed_interaction, behaviors, indiv_list, undir_adj_table=False):
    ...
    table = adj_table(data, directed_interaction, behaviors,
                      indiv_list, undir_adj_table)
    dyad_obs = dyad_obs_time(data, indiv_list)
    adjdict = {key: None for key in behaviors}
    for i in behaviors:
        # The diagonals is emptied.
        # see if that is what we want.
