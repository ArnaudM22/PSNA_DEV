...
time_indiv = tonkean.data.groupby('Observation id').first().groupby('Subject').sum().loc[:,'focal_length']
time_indiv.plot.pie()
plt.show()
plt.boxplot(time_indiv)
plt.title("Distribution of the total observation length for each individual")
plt.show()
