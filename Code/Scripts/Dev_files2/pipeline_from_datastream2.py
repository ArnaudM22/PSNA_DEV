...
data = pd.read_excel('../Data/Cleaned_data_input_format/sai2023_test2.xls')
...
social_behaviors = affiliative_interactions + \
    agonistic_interactions + proximity_association + ['Grab Tail']
...
behavior_rate_dict = pars.table_dict(data, social_behaviors,
...
with pd.ExcelWriter('layers_sai4.xlsx') as writer:
    for df_name, df in behavior_rate_dict.items():
        df.to_excel(writer, sheet_name=df_name)

dsi.to_excel('dsi.xls')
