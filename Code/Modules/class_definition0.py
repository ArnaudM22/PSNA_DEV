...
class Focals:
    ...
    def __preprocessing(self, path, check_empty_col, ignore, save, open_preprocessed, start_end):
        ...
        if open_preprocessed == True:
            ...
            data = order_data(path, start_end)
            if 'correct_behavioral_cat' not in ignore:
                data, behav_cor = correct_behavioral_cat(data, self.reference_behavior_table)
            data = new_behavioral_cat(data)
            if 'empty_col' not in ignore:
                data, empty_col_value = clean.empty_col(data, check = True)
                
            self.data = data
            self.behav_cor = behav_cor
            ...
            ###
            empty_col(data, check=check_empty_col)
            ###
            if 'empty_col' not in ignore:
                ###
